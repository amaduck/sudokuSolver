...
""" Methods of assigning numbers to grid """
...
def populate_grid():
    # Prompts for values in console - slow and annoying, dashes need to be entered
    print("Please enter the starting values")
    print("If a cell is blank, enter -")
    for x in range(rows):
        for y in range(cols):
            input_string = "Enter value for cell (" + str(x) + "," + str(y) + ")"
            valid = False
            while not valid:
    ...
    mark_passed()  # notes whether values known at start


def number_entry():
    # Takes number via GUI
...
def check_csv():
    # Check csv file before importing to ensure it's ok
...
def import_csv():
    # Imports values from CSV file
    # Uses import.csv, in root directory
    # Program calls check_csv() before this, to ensure csv file only contains ints between 1 and 9
    with open("import.csv") as testFile:
        x = 0
        for line in testFile:
            line = line.strip()
...
""" Methods to display grids """
def print_potentials():
    # Prints the grid of potential values
    for x in range(9):
        for y in range(9):
...
def print_full_grid(grid):
    # Prints the passed grid in console
    print("-" * 25)
    for row in range(9):
        print("| ", end="")
        for col in range(9):
